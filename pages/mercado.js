...
// Mercado & Clima — v1.0  |  16 Abr 2026
// Padrão: Router.register('mercado', function(main) { main.innerHTML = html; ... })
// ============================================================
...
Router.register('mercado', function(main) {
  ...
  function hb(title, body, good, bad) {
    return '<div class="mc-hb">?<div class="mc-ht">' +
      ...
      '<span class="mc-ht-g">' + good + '</span>' +
      '<span class="mc-ht-r">' + bad  + '</span>' +
    '</div></div>';
  }

  function ctxCard(cls, lbl, icon, val, valSm, sub, subBold, bdgCls, bdgTxt, insight, htT, htB, htG, htR) {
    var subHtml = sub
    ...
    return '<div class="mc-cc ' + cls + '">' +
      '<div class="mc-cc-top">' +
        ...
        '<div class="mc-cc-icons">' + hb(htT, htB, htG, htR) + '<span class="mc-cc-icon">' + icon + '</span></div>' +
      '</div>' +
  ...
  function cd(extraCls, num, events, tipId) {
    var evHtml = (events || []).map(function(e) {
      return '<div class="mc-cev ' + e[0] + '">' + e[1] + '</div>';
    }).join('');
    var numHtml = extraCls === 'today'
      ? '<span style="background:var(--navy);color:#fff;width:20px;height:20px;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;font-size:11px">' + num + '</span>'
      : num;
  ...
  '<div class="section-block anim d1 mc-ctx-block">' +
    '<div class="mc-ctx-grid">' +
      ctxCard('c1','Clima Hoje','☀️','33°C',false,'Patrocínio · Sol · Min 19°C',false,'wn','Sáb 36°C previsto','Calor no fim de semana → demanda churrasco ALTA. Monitorar cadeia do frio.','Clima Hoje','Temperatura atual na cidade selecionada. Fonte: OpenWeather API.','✓ Abaixo 28°C: clima neutro','✗ Acima 32°C: atenção na cadeia do frio') +
      ctxCard('c2','Próxima Data','📅','Dia das Mães',true,'11 dias',true,'wn','Alto impacto · 11/05','Iniciar comunicação no balcão esta semana. Kit churrasco família.','Datas Estratégicas','Próxima data comemorativa relevante para açougue e rotisseria.','✓ Com 7+ dias: tempo hábil','✗ Menos de 3 dias: ação emergencial') +
      ctxCard('c3','Boi Gordo','🐄','366<span class="mc-unit">/@</span>',false,'',false,'up','▲ +4,2% no mês','Tendência de alta — revisar preço do traseiro esta semana.','Boi Gordo (CEPEA)','Cotação R$/arroba em SP. R$10/@ de alta ≈ +R$0,67/kg na carne.','✓ Estável/queda: margem protegida','✗ Alta 2+ meses: revisar preço do traseiro') +
      ctxCard('c4','Suíno Vivo','🐷','6,96<span class="mc-unit">/kg</span>',false,'',false,'dn','▼ -2,8% semana','Janela de compra aberta — reforçar mix suíno agora.','Suíno Vivo (CEPEA)','Cotação R$/kg posto SP. Base de custo para costelinha, lombo e pernil.','✓ Em queda: janela de compra aberta','✗ Acima R$8,00: revisar preços') +
      ctxCard('c5','Frango Cong.','🐔','8,10<span class="mc-unit">/kg</span>',false,'',false,'nt','↔ Estável','Margem protegida · Favorável para rotisseria.','Frango Atacado (CEPEA)','Cotação R$/kg frango congelado atacado SP. Referência para rotisseria.','✓ Estável: margem previsível','✗ Alta acima 10%: revisar frango assado') +
      ctxCard('c6','IPCA Alim.','📊','+0,9%',false,'Acum. 12m: +8,1%',false,'up','▲ Acima da meta','Consumidor pressionado — priorizar mix de valor percebido alto.','IPCA Alimentação (IBGE)','Variação mensal dos preços de alimentos. 2× acima do geral = consumidor migra para cortes baratos.','✓ Abaixo 0,5%/mês: pressão baixa','✗ Acima 0,8%/mês: reforce mix de valor') +
      ctxCard('c7','Boi Futuro B3','📉','342<span class="mc-unit">/@</span>',false,'Mai/26 · B3',false,'dn','▼ -6,6% vs físico','Mercado prevê queda em maio — avaliar adiar compra de boi gordo.','Contrato Futuro Boi (B3)','Preço que o mercado espera para o boi gordo. Futuro abaixo físico = mercado prevê queda.','✓ Futuro abaixo físico: aguardar para comprar','✗ Futuro acima físico: antecipar compra agora') +
      ctxCard('c8','Export. Bovina','🌍','+15,1%',true,'vs Abr/2025 · COMEX',false,'up','▲ Recorde Abr/26','Exportação recorde → pressão de alta no boi. Antecipar compras bovinas.','Exportação Carne Bovina (COMEX)','Exportação em alta = frigoríficos preferem mercado externo = boi interno tende a subir.','✓ Estável/queda: oferta interna normal','✗ Em alta: boi interno tende a subir') +
      ctxCard('c9','Próx. Pagamento','💰','Dia 20',true,'4 dias',true,'wn','Pico de demanda','Entrada de salário em 4 dias — garantir estoque de bovino e suíno antes.','Calendário de Pagamentos','Dias 5, 10, 15 e último útil do mês concentram maior fluxo de compras de carne no varejo.','✓ Com 3+ dias: preparar estoque antecipado','✗ Amanhã: último momento para reposição') +
    '</div>' +
  '</div>' +
  ...
  '<div class="section-block anim d2 mc-cot-block">' +
    ...
    '<div class="mc-cot-grid">' +
      ...
      '<div class="mc-cot-card">' +
        ...
        '<div class="mc-cot-badges"><span class="mc-badge up">▲ +4,2% mês</span><span class="mc-badge up">▲ +11,8% trim.</span></div>' +
        '<div class="mc-chart-wrap"><canvas id="mc-cvs-boi"></canvas></div>' +
        '<table class="mc-tbl" id="mc-tbl-boi"><colgroup id="mc-cg-boi"></colgroup><tbody>' +
          '<tr><td class="mc-tlbl">Valor</td>' + COT.boi.vals.map(function(v){ return '<td class="mc-tval">'  + f0(v) + '</td>'; }).join('') + '</tr>' +
          '<tr><td class="mc-tlbl">Var.</td>'  + varRow(COT.boi.vals, f0) + '</tr>' +
          '<tr><td class="mc-tlbl">Var.%</td>' + pctRow(COT.boi.vals) + '</tr>' +
        '</tbody></table>' +
      ...
      '<div class="mc-cot-card">' +
        ...
        '<div class="mc-cot-badges"><span class="mc-badge dn">▼ -2,8% sem.</span><span class="mc-badge dn">▼ -1,2% mês</span></div>' +
        '<div class="mc-chart-wrap"><canvas id="mc-cvs-suino"></canvas></div>' +
        '<table class="mc-tbl" id="mc-tbl-suino"><colgroup id="mc-cg-suino"></colgroup><tbody>' +
          '<tr><td class="mc-tlbl">Valor</td>' + COT.suino.vals.map(function(v){ return '<td class="mc-tval">'  + f2(v) + '</td>'; }).join('') + '</tr>' +
          '<tr><td class="mc-tlbl">Var.</td>'  + varRow(COT.suino.vals, f2) + '</tr>' +
          '<tr><td class="mc-tlbl">Var.%</td>' + pctRow(COT.suino.vals) + '</tr>' +
        '</tbody></table>' +
      ...
      '<div class="mc-cot-card">' +
        ...
        '<div class="mc-cot-badges"><span class="mc-badge nt">↔ +0,1% sem.</span><span class="mc-badge nt">↔ Estável 3m</span></div>' +
        '<div class="mc-chart-wrap"><canvas id="mc-cvs-frango"></canvas></div>' +
        '<table class="mc-tbl" id="mc-tbl-frango"><colgroup id="mc-cg-frango"></colgroup><tbody>' +
          '<tr><td class="mc-tlbl">Valor</td>' + COT.frango.vals.map(function(v){ return '<td class="mc-tval">'  + f2(v) + '</td>'; }).join('') + '</tr>' +
          '<tr><td class="mc-tlbl">Var.</td>'  + varRow(COT.frango.vals, f2) + '</tr>' +
          '<tr><td class="mc-tlbl">Var.%</td>' + pctRow(COT.frango.vals) + '</tr>' +
        '</tbody></table>' +
  ...
  '<div class="section-block anim d3 mc-ipca-block">' +
    ...
    '<div class="mc-ipca-layout">' +
      '<div class="mc-ipca-bns">' +
        ...
        ipcaBn('fr','Crítico','r','IPCA Alimentação','red','<strong>+8,1%</strong>','Acumulado 12m · Acima do geral','IPCA Alimentação','Variação de preços de alimentos. 2× acima do geral = consumidor migra para cortes baratos.','✓ Próximo ao geral: pressão equilibrada','✗ 2× acima: consumidor corta cortes nobres') +
        ipcaBn('fr','Crítico','r','IPCA Carnes','red','<strong>+10,3%</strong>','Acumulado 12m · 2× acima do geral','IPCA Carnes','Subcomponente específico para carnes. Quanto o consumidor final pagou a mais.','✓ Abaixo de 5%: consumidor compra com conforto','✗ Acima de 8%: promova proteínas alternativas') +
        ipcaBn('fb','Monitorar','b','Dólar Comercial','blue','<strong>R$ 5,72</strong>','Comercial venda · Banco Central','Dólar Comercial (BCB)','Câmbio do BCB. Dólar alto encarece cortes exportáveis: picanha e alcatra.','✓ Abaixo R$5,50: pressão cambial baixa','✗ Acima R$6,00: risco de alta nos cortes nobres') +
      '</div>' +
  ...
  '<div class="section-block anim d4 mc-cal-block">' +
    ...
    '<div class="mc-cal-grid">' +
      ...
      cd('','1',[['feriado','Tira-dentes']]) +
      cd('','2') + cd('','3') +
      cd('','4',[['clima','☀ 36°C']]) +
      cd('','5',[['pagto','💰 Pagto. 5']]) +
      cd('','6') + cd('','7') + cd('','8') + cd('','9') +
      cd('','10',[['feriado','Paixão de Cristo'],['pagto','💰 Pagto. 10']]) +
      cd('','11') +
      cd('','12',[['feriado','Páscoa']]) +
      cd('','13') + cd('','14') +
      cd('','15',[['pagto','💰 Pagto. 15']]) +
      cd('today','16') +
      cd('','17') + cd('','18',[['clima','☀ 34°C']]) + cd('','19') +
      cd('pagto','20',[['pagto','💰 Pagto. FGTS']]) +
      cd('','21',[['feriado','Tiradentes']]) +
      cd('','22',[['sazon','Dia da Terra']]) +
      cd('','23',[['custom','Reunião Rede']]) +
      cd('','24') +
      cd('calhov','25',[['sazon','🐷 Dia Suíno']],null,'tip-suino') +
    ...
    '<div class="mc-mes-insight"><span class="mc-mes-icon">🧭</span><div><div class="mc-mes-ttl">Análise de Abril 2026</div><div class="mc-mes-body"><strong>3 feriados + 5 picos de pagamento</strong> neste mês. Páscoa exige mix focado em aves e suíno. Dia da Carne Suína (25/04) + FGTS dia 20 = janela dupla para suíno. Exportação bovina recorde pressiona preço do boi no curto prazo.</div><div class="mc-mes-action">→ Prioridade: Suíno (preço + data + pagamento) · Aves (Páscoa + Rotisseria) · Bovino (antecipar revisão de preço)</div></div></div>' +
  '</div>' +
  ...
  '<div class="section-block anim d5 mc-exp-block">' +
    '<div class="mc-exp-ctrl">' +
      '<span class="mc-el">Linhas</span><select class="filter-select"><option>Produto</option><option>Mês</option></select>' +
      '<span class="mc-el">Colunas</span><select class="filter-select"><option>Mês</option><option>Semana</option></select>' +
      '<span class="mc-el">Métrica</span><select class="filter-select"><option>Cotação Média</option><option>Variação %</option></select>' +
      '<button class="mc-exp-export">↓ Excel</button>' +
    '</div>' +
    '<div style="overflow-x:auto"><table class="mc-pvt">' +
      '<thead><tr><th style="text-align:left">Produto</th><th>Nov/25</th><th>Dez/25</th><th>Jan/26</th><th>Fev/26</th><th>Mar/26</th><th>Abr/26</th><th style="background:rgba(201,168,76,0.22);color:#6A4C00">Média</th></tr></thead>' +
      '<tbody>' +
      ...
      '</tbody>' +
      '<tfoot><tr><td>Var. Período</td><td>—</td><td><span class="hc u1" style="font-size:10px">+2,1%</span></td><td><span class="hc u1" style="font-size:10px">+1,8%</span></td><td><span class="hc u2" style="font-size:10px">+3,4%</span></td><td><span class="hc u1" style="font-size:10px">+2,3%</span></td><td><span class="hc u2" style="font-size:10px">+2,9%</span></td><td style="font-size:10px;color:var(--t3)">*parcial</td></tr></tfoot>' +
    '</table></div>' +
  ...
  // ── COTAÇÕES: Chart.js + alinhamento regra Dossiê v3.10 ───
  ...
  function buildCotChart(id, data, lc, fc, fmtFn) {
    ...
    var chart = new Chart(cvs, {
      ...
      options: {
        responsive: true,
        maintainAspectRatio: false,  // essencial: respeita altura do container
        plugins: {
          legend: { display: false },
          tooltip: { callbacks: { label: function(ctx) { return ' ' + fmtFn(ctx.parsed.y); } } }
        },
        scales: {
          x: { display: false, offset: false },  // offset:false = pontos nas extremidades
  ...
  // ── IPCA BARS ─────────────────────────────────────────────
  (function() {
    var el = main.querySelector('#mc-ipca-bars');
    if (!el) return;
  ...
  (function() {
    ...
    if (!aw) return;
    var defs = [
      { cond: ul.c > 0.9,                        cls: 'r', ttl: '⚠ Crítico — IPCA Carnes',      body: 'Carnes sobem <strong>2× mais</strong> que a inflação geral nos últimos 6 meses. Reforce sobrecoxa, coxão mole e linguiça.' },
      ...
      { cond: 5.72 > 5.5,                        cls: 'b', ttl: '◉ Monitorar — Câmbio',         body: 'Dólar em <strong>R$ 5,72</strong>, acima do patamar neutro de R$ 5,50. Cortes exportáveis (picanha, alcatra) podem sofrer pressão.' }
    ];
  ...
  main.querySelectorAll('.mc-cd.calhov').forEach(function(day) {
    day.addEventListener('mouseenter', function() {
      var tip = main.querySelector('#' + this.getAttribute('data-tip'));
      if (!tip) return;
    ...
    day.addEventListener('mouseleave', function() {
      main.querySelectorAll('.mc-cal-tip').forEach(function(t) { t.classList.remove('mc-on'); });
    });
  });
