...
// Bloco 2: tendência — identidade visual idêntica ao Equipe
//   - linha: borderWidth 2.5, tension 0.35, fill rgba(.06)
//   - pontos: radius 3, borderWidth 2, borderColor #fff
//   - datalabels: font 600 10px Outfit, cor #1F2937, y-10
//   - padding: dinâmico colW=44 + offset:false (fórmula equipe)
//   - tabela: só CONF + VAR, cores semânticas, 8px/600
// ============================================================
...
Router.register('temperatura', async function(container) {
    ...
    return (
      '<div style="display:flex;align-items:stretch;gap:4px;margin:10px auto 8px;justify-content:center;">' +
        ...
        '<div style="position:relative;width:8px;height:90px;background:rgba(255,255,255,.08);border-radius:4px;overflow:visible;flex-shrink:0;">' +
          '<div style="position:absolute;left:0;right:0;bottom:' + zBot + '%;height:' + zH + '%;background:rgba(255,255,255,.15);border-radius:3px;overflow:hidden;"></div>' +
        ...
        '<div class="tmp-tc ' + eq.id + '">' +
          ...
          '<div class="tmp-tc-metrics">' +
            ...
            '<div class="tmp-tc-conf">' +
              '<div class="tmp-tc-donut-wrap"><canvas id="tmpDn_' + eq.id + '"></canvas>' +
              '<span class="tmp-tc-donut-center" style="color:' + dcc + '">' + eq.confPct + '%</span></div>' +
  ...
  // ══════════════════════════════════════════════════════════════
  // BLOCO 2 — TENDÊNCIA (identidade visual = equipe)
  // ══════════════════════════════════════════════════════════════
  function renderBloco2() {
    return '<div class="section-block trend anim d2">' +
      ...
      '<div class="tmp-trend-grid">' +
        '<div class="tmp-trend-card">' +
          '<div class="tmp-trc-title">Conformidade Mensal</div>' +
          '<div class="tmp-trc-bn-row"><span class="tmp-trc-bn" id="tmpBn1"></span><span class="tmp-trc-var" id="tmpVar1"></span></div>' +
          '<div class="tmp-trc-chart"><canvas id="tmpCh1"></canvas></div>' +
          '<table class="tmp-trc-tbl" id="tmpTb1"></table>' +
        '</div>' +
        '<div class="tmp-trend-card">' +
          '<div class="tmp-trc-title">Ultimas 8 Semanas</div>' +
          '<div class="tmp-trc-bn-row"><span class="tmp-trc-bn" id="tmpBn2"></span><span class="tmp-trc-var" id="tmpVar2"></span></div>' +
          '<div class="tmp-trc-chart"><canvas id="tmpCh2"></canvas></div>' +
          '<table class="tmp-trc-tbl" id="tmpTb2"></table>' +
        '</div>' +
        '<div class="tmp-trend-card">' +
          '<div class="tmp-trc-title">Ultimos 7 Dias</div>' +
          '<div class="tmp-trc-bn-row"><span class="tmp-trc-bn" id="tmpBn3"></span><span class="tmp-trc-var" id="tmpVar3"></span></div>' +
          '<div class="tmp-trc-chart"><canvas id="tmpCh3"></canvas></div>' +
          '<table class="tmp-trc-tbl" id="tmpTb3"></table>' +
        '</div>' +
      '</div></div>';
  }

  // Plugin datalabels — igual ao equipe (afterDatasetsDraw)
  var dlPlugin = {
    id: 'tmpDl',
    afterDatasetsDraw: function(chart) {
      ...
      chart.data.datasets.forEach(function(ds, i) {
        chart.getDatasetMeta(i).data.forEach(function(pt, idx) {
          ...
          ctx.fillText(ds.data[idx] + '%', pt.x, pt.y - 6);
          ctx.restore();
        });
      });
    }
  ...
  function initBloco2Charts(tendencia, eq) {
    ...
    sets.forEach(function(s) {
      ...
      // Badge variação — igual ao equipe
      ...
      var nTk    = labels.length;

      // Fórmula de padding EXATA do equipe:
      // colW=44, dW=(canvas.width-colW)/nTk, pL=colW+dW/2, pR=dW/2
      var colW = 44;
      var dW   = (canvas.width - colW) / nTk;
      var pL   = Math.round(colW + dW / 2);
      var pR   = Math.round(dW / 2);

      // yMin — igual ao equipe (0 se tiver zeros)
      var nonZero = vals.filter(function(v){ return v > 0; });
      var hasZeros = vals.some(function(v){ return v === 0; });
      ...
      var ch = new Chart(canvas.getContext('2d'), {
        ...
        data: {
          ...
          datasets: [{
            data: vals,
            // Identidade visual IDÊNTICA ao equipe:
        ...
        options: {
          ...
          scales: {
            ...
            x: {
              display: true,
              offset:  false,        // pontos nas extremidades
              grid:    { display: false },
      ...
      // Tabela: CONF + VAR, sem LEIT
      // colgroup: col 0 = 44px, demais = auto (table-layout fixed distribui)
  ...
  function initBloco2BCharts(leituras) {
    ...
    ['balcao','resf','cong'].forEach(function(eqId) {
      ...
      ['m','s','d'].forEach(function(per) {
        ...
        var yPad = iC ? 4 : 2;
        var yMin = allV.length ? Math.min(fMin-yPad, Math.min.apply(null,allV)-yPad) : fMin-yPad;
        var yMax = allV.length ? Math.max(fMax+yPad, Math.max.apply(null,allV)+yPad) : fMax+yPad;
        var ch = new Chart(canvas.getContext('2d'), {
          type: 'line',
          ...
          options: { responsive:true, maintainAspectRatio:false,
            plugins:{ legend:{display:false}, datalabels:{display:false},
              tooltip:{callbacks:{label:function(ctx){ if(ctx.datasetIndex>0) return null; var v=ctx.parsed.y; return v!==null?v.toFixed(1)+'\u00b0C '+(isOk(v)?'\u2713':'\u26a0'):''; }}}},
            scales:{
  ...
  function renderBloco4(ranking) {
    ...
    function eqB(d) {
      var v=d.delta, vc=v>0?'up':(v<0?'down':'eq');
  ...
  function initPillEvents() {
    document.querySelectorAll('#tmpTrendPills .eq-pill').forEach(function(el) {
      el.addEventListener('click', function() {
        ...
        // Destruir apenas charts de tendência (não os de leituras)
        // Re-renderizar apenas o bloco 2
        var contentEl = document.getElementById('tempContent');
        if (!contentEl || !tmpData) return;
        // Destruir charts de tendência
        tmpCharts = tmpCharts.filter(function(c) {
          if (c.canvas && (c.canvas.id==='tmpCh1'||c.canvas.id==='tmpCh2'||c.canvas.id==='tmpCh3')) {
            c.destroy(); return false;
          }
          return true;
        });
        // Resetar tabelas
        ['tmpTb1','tmpTb2','tmpTb3'].forEach(function(id){
          var t=document.getElementById(id); if(t) t.innerHTML='';
        });
